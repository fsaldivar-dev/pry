    ...
    public static func colorizeJSON(_ json: String) -> String {
        ...
        while i < json.endIndex {
            ...
            if c == "\"" {
                if !inString {
                    inString = true
                    let color = isKey ? "\u{001B}[36m" : "\u{001B}[32m"  // cyan for keys, green for strings
                    result += color + "\""
                } else {
                    inString = false
                    result += "\"\u{001B}[0m"
                ...
                if c == ":" {
                    ...
                    result += "\u{001B}[34mtrue\u{001B}[0m"  // blue
                    i = json.index(i, offsetBy: 4)
                    continue
                } else if json[i...].hasPrefix("false") {
                    result += "\u{001B}[34mfalse\u{001B}[0m"  // blue
                    i = json.index(i, offsetBy: 5)
                    continue
                } else if json[i...].hasPrefix("null") {
                    result += "\u{001B}[90mnull\u{001B}[0m"  // gray
                    i = json.index(i, offsetBy: 4)
                    continue
                } else {
                    result.append(c)
