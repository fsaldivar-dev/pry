...
public struct DiffTool {
    ...
    public static func diff(req1: RequestStore.CapturedRequest, req2: RequestStore.CapturedRequest) -> [DiffLine] {
        ...
        if req1.method == req2.method {
            lines.append(.same("Method: \(req1.method)"))
        } else {
            lines.append(.changed(label: "Method", left: req1.method, right: req2.method))
        }

        // URL
        if req1.url == req2.url {
            lines.append(.same("URL: \(req1.url)"))
        } else {
            lines.append(.changed(label: "URL", left: req1.url, right: req2.url))
        }
        ...
        if req1.requestBody != req2.requestBody {
            if let b1 = req1.requestBody {
                lines.append(.removed("Body: \(b1.prefix(200))"))
            }
            if let b2 = req2.requestBody {
                lines.append(.added("Body: \(b2.prefix(200))"))
            }
        }
        ...
        if req1.responseBody != req2.responseBody {
            if let b1 = req1.responseBody {
                lines.append(.removed("Response: \(b1.prefix(200))"))
            }
            if let b2 = req2.responseBody {
                lines.append(.added("Response: \(b2.prefix(200))"))
            }
        }
